...
const itemSku=i=>String(i?.sku||i?.SKU||i?.mahang||i?.product_sku||i?.productSku||i?.item_sku||i?.itemSku||i?.variation_sku||i?.variationSku||'').trim();

/* Increase API page size on high-volume calendar dates.
   Special dates: 1, 10, 15, 20, 25 and same day/month (e.g. 9/9).
   If a requested date range contains at least one special date, limit is at
   least 400. Existing larger limits (such as monthly 1000) are preserved. */
...
/* Patch the real report renderer at the data level. The original renderer uses
   `const key=sku||name`; we normalize every equal SKU to one identical display SKU
   BEFORE that Map is created, so revenue/profit/orders are genuinely aggregated. */
function patchReport(){
 if(typeof window.renderReport!=='function'||window.renderReport.__skuGrouped)return;
 const original=window.renderReport;
 function grouped(rows){
  const canonical=new Map();
  const out=(Array.isArray(rows)?rows:[]).map(r=>{
   if(!r||typeof r!=='object')return r;
   const copy={...r};let list=r.eoi_listproduct;
   try{list=typeof list==='string'?JSON.parse(list):list}catch(_){list=[]}
   if(list&&typeof list==='object'&&!Array.isArray(list))for(const k of['items','products','list'])if(Array.isArray(list[k])){list=list[k];break}
   if(Array.isArray(list)){
    copy.eoi_listproduct=list.map(it=>{
     if(!it||typeof it!=='object')return it;
     const x={...it},raw=itemSku(it),key=skuKey(raw);
     if(key){const shown=canonical.get(key)||raw;canonical.set(key,shown);x.sku=shown;x.mahang=shown;x.product_sku=shown}
     return x;
    });
   }
   return copy;
...
css();patchPayloadLimit();patchReport();const ob=new MutationObserver(()=>{install();paintRates();patchPayloadLimit();patchReport()});ob.observe(document.documentElement,{childList:true,subtree:true});install();paintRates();setInterval(()=>{patchPayloadLimit();patchReport()},250);
})();